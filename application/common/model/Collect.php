        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif( mac_array_filter($filter_arr,$v['vod_name']) !==false) {
                ...
                if (!$info) {
                    ...
                    if($param['opt'] == 2){
                        $des= '数据操作没有勾选新增，跳过。';
                    }elseif(empty((string)join('$$$', $collect_filter['play'][$param['filter']]['cj_play_from_arr'])) && empty($v['vod_play_from'])){  //自己添加的//后面表示头车不判断
                        $des= '播放组为空，跳过。(' . (string)join('$$$', $collect_filter['play'][$param['filter']]['cj_play_from_arr']). ')';
                    }elseif($v['vod_year'] < 1){  //自己添加的//后面表示头车不判断
                        $des= '年代为空，跳过。';
                    }else {
                                ...
                                if ($cj_play_url == $info['vod_play_url']) {
                                    ...
                                    if ($arr1[$play_key] == $cj_play_url) {
                                        ...
                                        if ($config['urlrole'] == 1) {
                                            ...
                                            $des .= '播放组合并(' . $cj_play_from . ')，更新ok。';
                                            $ec=true;
                                        }else{
                                            if(count($tmp2) > count($tmp1)){
                                                $arr1[$play_key] = $cj_play_url;     //如采集技术比数据库里的数量多则替换,否则不替换
                                                $des .= '播放组替换(' . $cj_play_from . ')，更新ok。';
                                                $ec=true;
                                                if($v['type_id'] != 1){
                                                  model('Message')->sendWeAppMessage($info,count($tmp2));
                                                }
                                                //print_r($asd);
                                                
                                            }else{
                                                $des .= '播放组替换(' . $cj_play_from . ')，无需更新。';  
                                            } 
                                        }
                                    }
